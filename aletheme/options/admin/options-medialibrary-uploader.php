...
if ( ! function_exists( 'optionsframework_mlu_get_silentpost' ) ) {
	...
	function optionsframework_mlu_get_silentpost ( $_token ) {
		...
		if ( $_token ) {
			...
			// В версии языка PHP 7.2 
			// вызов count() с параметром, который является скалярным,
			// null или объектом, который не реализовал интерфейс Countable, 
			// будет выдавать Warning.
			if ( count( $_posts ) ) {
				$_id = $_posts->ID;
			} else {
